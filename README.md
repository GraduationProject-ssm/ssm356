...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=152)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本智慧养老平台也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让老人信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，智慧养老平台的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，智慧养老平台是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此智慧养老平台为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
智慧养老平台的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
图4.1 程序工作的原理图
## 4.2 系统功能结构设计
在分析并得出使用者对程序的功能要求时，就可以进行程序设计了。如图4.2展示的就是管理员功能结构图，管理员主要负责填充老人和其类别信息，并对已填充的数据进行维护，包括修改与删除，管理员也需要对公告,服务管理,留言管理等进行维护。
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|8|laoren\_id|Integer|老人|是|
|9|jigoushequ\_id|Integer|服务人员|是|
|10|fuwu\_time|Date|服务时间|是|
|11|insert\_time|Date|申请时间|是|
|12|fuwujilu\_content|String|服务详情|是|
|13|fuwujilu\_types|Integer|评价星级类型|是|
|14|pingjia\_content|String|评价详情|是|
|15|update\_time|Date|评价时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|22|laoren\_id|Integer|老人|是|
|23|jiaren\_name|String|家人姓名|是|
|24|jiaren\_phone|String|家人手机号|是|
|25|jiaren\_id\_number|String|家人身份证号|是|
|26|jiaren\_photo|String|照片|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|28|jigoushequ\_name|String|姓名|是|
|29|jigoushequ\_phone|String|手机号|是|
|30|jigoushequ\_id\_number|String|身份证号|是|
|31|jigoushequ\_photo|String|照片|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|33|laoren\_id|Integer|老人|是|
|34|jiuzhenjilu\_types|Integer|就诊类型|是|
|35|yiyuan|String|就诊医院|是|
|36|yisheng|String|就诊医生|是|
|37|yisheng\_phone|String|医生手机号|是|
|38|insert\_time|Date|就诊时间|是|
|39|jiuzhenjilu\_content|String|就诊详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|41|laoren\_name|String|老人姓名|是|
|42|laoren\_phone|String|老人手机号|是|
|43|laoren\_id\_number|String|老人身份证号|是|
|44|yonghu\_photo|String|照片|是|
|45|duju\_types|Integer|是否独居|是|
|46|canji\_types|Integer|是否残疾|是|
|47|yanglaofanghsi\_types|Integer|养老方式类型|是|
|48|jibing\_content|String|慢性疾病|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|50|laoren\_id|Integer|老人|是|
|51|liuyan\_name|String|留言名称|是|
|52|liuyan\_types|Integer|留言类型|是|
|53|insert\_time|Date|留言时间|是|
|54|liuyan\_content|String|留言详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|56|laoren\_id|Integer|老人|是|
|57|qiuzhu\_name|String|求助名字|是|
|58|qiuzhu\_types|Integer|求助类型|是|
|59|zhixing\_time|Date|执行时间|是|
|60|insert\_time|Date|申请时间|是|
|61|qiuzhu\_content|String|求助详情|是|
|62|qiuzhu\_yesno\_types|Integer|是否处理|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|64|laoren\_id|Integer|老人|是|
|65|jigoushequ\_id|Integer|随访人员|是|
|66|insert\_time|Date|随访时间|是|
|67|suifangjilu\_content|String|随访详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|69|yule\_name|String|活动名称|是|
|70|yule\_types|Integer|活动类型|是|
|71|insert\_time|Date|通知时间|是|
|72|yule\_content|String|活动详情|是|
|73|create\_time|Date|创建时间|是|
表4.12管理员表表
...
## 5.1 管理员功能介绍
### 5.1.1 老人管理
如图5.1显示的就是老人管理页面，此页面提供给管理员的功能有：新增老人,修改老人,删除老人。
...
图5.2 公告管理页面
### 5.1.3 家人管理
如图5.3显示的就是家人管理页面，此页面提供给管理员的功能有：新增家人,修改家人,删除家人。
